...
Além desta introdução, este trabalho está dividido em mais \total{numchapters} cinco capítulos:

\begin{description}
  \item[\autoref{chap:revision} - \nameref{chap:revision}:] Esse capítulo discute um contexto pedagógico
  ...
  \item[\autoref{chap:software} - \nameref{chap:software}:] Nesse capítulo são descritas as fases de especificação, projeto, implementação e testes do sistema.

  \item[\autoref{chap:rsa} - \nameref{chap:rsa}:] Nesse capítulo é apresentado o software desenvolvido,
  definições, as principais telas e característica do sistema desenvolvido.

  \item[\autoref{chap:experimentos} - \nameref{chap:experimentos}:] Nesse capítulo são apresentados e discutidos os testes de usabilidade
  realizados para validar o software desenvolvido, apresentado no \autoref{chap:rsa}.
  Os procedimentos, material e métodos, resultados e discussões e cada experimento também é apresentado nesse capítulo.

  \item[\autoref{chap:final_thoughts} - \nameref{chap:final_thoughts}:] São realizadas as considerações finais do
  trabalho e apontadas diretrizes para trabalhos futuros.

\end{description}
\total{numappendixes} Apêndices foram criados para complementar o conteúdo deste trabalho. Eles estão organizados da seguinte forma:
\begin{description}
  \item[\autoref{appendix:sus} - \nameref{appendix:sus}:] Questionário SUS disponibilizado aos professores e alunos
  referente aos testes de usabilidade.
  \item[\autoref{appendix:tasks} - \nameref{appendix:tasks}:] Descrição das tarefas utilizadas nos testes de
  usabilidade com os professores.
\end{description}
