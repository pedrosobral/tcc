\section{Processos de \textit{software}}

Engenharia de \textit{software} pode ser definida como:
\begin{citacao}[english]
1. the systematic application of scientific and technological knowledge, methods, and experience to the design,
implementation, testing, and documentation of \textit{software} [...]  2. the application
of a systematic, disciplined, quantifiable approach to the development, operation,
and maintenance of \textit{software}; that is, the application of engineering to \textit{software} \cite{IEEE2010}.
\end{citacao}

A engenharia de \textit{software} deve ter foco na qualidade, que apoia as outras camadas
...
como finalidade garantir a efetiva utilização da tecnologia engenharia de \textit{software}, que dessa forma
leva à produção de um \textit{software}. Os detalhes de como fazer o \textit{software} pertencem
a camada de métodos. Os métodos da engenharia de \textit{software} incluem tarefas de planejamento
e estimativa de \textit{software}, análise de requisitos, modelagem de projeto, codificação,
testes e manutenção. As ferramentas de engenharia de \textit{software} auxiliam as camadas de
processo e métodos, com ferramentas automatizadas, que por sua vez, quando integradas,
é estabelecido um suporte ao desenvolvimento de \textit{software} chamado CASE -
\textit{Computer Aided \textit{software} Engineering} \cite{Pressman2009, Sommerville2006}.

\begin{figure}[b]
  \centering
  \caption{Engenharia de \textit{software} - uma tecnologia em camadas}
...
fundamentais, a saber, especificação de \textit{software}, projeto e implementação de
\textit{software}, validação de \textit{software} e evolução de \textit{software}. Especificação de \textit{software}
ou engenharia de requisitos é uma fase importante e crítica do processo de engenharia
de \textit{software}. Importante porque é uma análise de requisitos bem feita que possibilitará
...
tendo como resultado um sistema executável. Depois, o \textit{software} deve ser verificado
para mostrar que atende às demandas dos usuários (validação do \textit{software}). Finalmente,
na fase de evolução de \textit{software}, o mesmo é modificado devido às mudanças
de requisitos e às necessidades dos usuários.

% Quanto ao modelos de engenharia de \textit{software} temos os modelos prescritivos e os ágeis.
% Os modelos prescritivos definem um conjunto de atividades explícitas para serem
% desenvolvidos no processo de engenharia de \textit{software}. O Processo Unificado de desenvolvimento
% tem o maior destaque e reconhencimento dentre os modelos prescritivos.
%
% \subsection{Processo Unificado}
% O Processo Unificado define quatro atividades no processo de desenvolvimento de \textit{software}.
...
%   desenvolvimento do \textit{software} também são esperados.
%   \item [{Construção:}] Os casos de uso UML desenvolvidos na fase de elaboração
%   são então desenvolvidos e testados. O \textit{software} deve estar funcionando
%   e documentado na conclusão dessa fase.
%   \item [{Transição:}] transferência do \textit{software} aos usuários finais.
% \end{description}
%
...
%   \item \emph{Implantação: }\textit{software} é distribuído aos usuários finais.
...
\begin{citacao}
  Estamos descobrindo melhores maneiras de desenvolver \textit{softwares}, fazendo-o e ajudando
...
conclusão de curso no curto prazo e os recursos limitados. Dessa forma, uma abordagem baseada em metodologias ágeis foi utilizada para %modelagem, documentação e codificação do \textit{software}.
especificação, projeto e implementação do \textit{software}. Nesse sentido, por exemplo,
na fase elicitação de requisitos não procurou-se a completa definição dos requisitos do \textit{software} e ela nem foi
...
Os resultados obtidos foram utilizados no desenvolvimento do \textit{software}.
...
de funcionar em \textit{smartphones}, \textit{tablets} e computadores independentemente
...
A metodologia ágil de \textit{software}, \textit{eXtreme Programming (XP)}, introduziu a
prática de expressar os requisitos de \textit{software} na forma de \textit{histórias de usuário},
que são descrições informais do que  o sistema deve fazer, evitando qualquer terminologia técnica \cite{Sommerville2006}.
...
A plataforma operacional escolhida para construção e operação do sistema levou em consideração
a utilização tecnologias, ferramentas e \textit{frameworks} \textit{open-source}.
...
\begin{description}
  ...
  Ela é uma linguagem de \textit{scripting} baseada em protótipos, multi-paradigma e dinâmica, suportando os estilos orientado a objetos, imperativo e funcional.
...
Um aplicativo desenvolvido no \textit{Ionic} é composto por um conjunto de \textit{pages} ou páginas.
...
\textit{Hooks} são técnicamente um \textit{middleware} ou funções que têm acesso aos
...
do \textit{software} por disponibilizar facilidades para o programador e implementar
...
Os itens da lista de tarefas da \autoref{fig:backlog} foram desenvolvidos de forma incremental.
...
Os critérios de aceitação foram escritos usando o formato utilizado no desenvolvimento dirigido por
...
\begin{description}
  \item[Dado que (\textit{Given}):] determinadas pré-condições são atendidas;
...
eles são \textit{autodescritivos}. As nomenclaturas do MongoDB diferem dos
...
um ou mais campos, e cada campo um valor de um tipo específico, incluindo vetores, dados
...
\begin{figure}[ht]
  ...
  \begin{lstlisting}[language=JavaScript]
  {
  	...
  	"name" : "Engenharia de \textit{software}",
  	"code" : "ENGSOFT123",
...
\section{Verificação do \textit{software}}

\subsection{Testes Automatizados}

Para garantir a qualidade do \textit{software} desenvolvido, desenvolveu-se testes
automatizados para avaliar se o \textit{software} estava sendo desenvolvido como esperado.
A base para desenvolver os testes automatizados foram os critérios de aceitação de cada história de usuário.
Alguns exemplos de critérios de aceitação foram descritos na \autoref{subsection:implementation}.
...
uma questão foi adicionada. Avançando no teste, outro formulário para adicionar questão é aberto
...
foi possível testar o \textit{software} de uma maneira abrangente ao envolver todos os usuários do sistema.
